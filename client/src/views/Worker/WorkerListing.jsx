...
function WorkerListing() {
    ...
    const getStocks = async () => {
        ...
        let data = await getApi('worker?'+filterUrl)
        if(data.status) {
            const params = new URLSearchParams(filterData);
            history.replace({ pathname: location.pathname, search: params.toString() });
            
            setStocks(data.resp.data.workers)
        }
    }
  ...
  return (
    <>
        {
            openFilter && <Filter filterSubmit={handleFilterSubmit} value={filterData} closeFilter={closeFilter}/>
        }
        {
            openSearch && <Search filterSubmit={handleSearchSubmit} value={filterData} closeSearch={closeSearch}/>
        }
